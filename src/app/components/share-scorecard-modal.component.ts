      ...
      <div class="flex flex-col gap-6 pt-2">
              ...
              @if (unlockedList().length > 0) {
                <div class="w-full">
                  ...
                  <div class="flex justify-center gap-2 flex-wrap">
                    @for (ach of unlockedList().slice(0, 5); track ach) {
                      <div class="w-8 h-8 flex items-center justify-center bg-white/20 rounded-full text-lg shadow-sm border border-white/30">
                        {{ ach.icon }}
                      </div>
                    }
                  </div>
        ...
        <div class="flex justify-end gap-3 mt-2">
          ...
          <button appButton (click)="handleDownload()" [disabled]="isCapturing()" class="bg-primary text-white hover:bg-primary-hover shadow-lg shadow-primary/30">
            @if (!isCapturing()) {
              <i-lucide name="download" class="w-5 h-5 mr-2"></i-lucide>
            }
            @if (isCapturing()) {
              <i-lucide name="loader-2" class="w-5 h-5 animate-spin mr-2"></i-lucide>
            }
            Download PNG
          </button>
        </div>
